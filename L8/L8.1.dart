...
void onCloseHandler() {
  print('⌚  Stream closed after ${STREAM_CLOSE_TIMEOUT}s');
}
...
Future delay(int seconds){
  ...
  // Console.write("⌚  ");
  // timer.start();
  // return Future.delayed(Duration(seconds: seconds)).then((_) {
  //   timer.stop();
  // });
  return new Future.delayed(Duration(seconds: seconds), () => print('⌚  ${seconds.toString()}s'));
}
void addAvengers(StreamController streamCtrl, List<dynamic> avengers) async{
  for(final avenger in avengers){
      try {        
        ...
        await streamCtrl.sink.add(avenger);
        await delay(AVENGER_GO_TIMEOUT);
      } catch (e) {
        if(streamCtrl.isClosed){
          print('${redText('☒ Stream is closed ☒')}');
          print('${redText('✖')} ${redText(avenger.runtimeType.toString())} is late ');
        }
        else{
          print(e);
        }       
  ...
  //     await streamCtrl.sink.add(avengers[i]);
  //     await delay(AVENGER_GO_TIMEOUT);
  //   } catch (e) {
  //     print('''
  //     ☒ Stream is closed ☒
  //     ✗ ${avengers[i].runtimeType.toString()} is late ✗
  //     ''');
  //     i = avengers.length;
  //   }
  // }
...
Future<List<dynamic>> transformAvengers(List<dynamic> avengers) async {
  ...
  streamSubscription = streamCtrl.stream.listen((avenger) {         
        if(isReadyGadget(avenger)) {
          ...
          print("${redText('✖')} Rejected: " + avenger.runtimeType.toString() + " -> without gadget");
      ...
      print("====> Transforming Avengers done");
    }, onError: (error) {
      print("Some Error");
    }
  );
